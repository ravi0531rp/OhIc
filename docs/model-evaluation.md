...
| RealBasicVSR | Designed for real-world degraded video and temporally aware restoration. | Actual tiny-clip inference passed on MPS; CPU fallback is implemented. CUDA remains unverified in OhIc. | Materially slower and more memory-intensive than the frame path; bounded windows cap temporal memory. Representative benchmarks remain necessary. | High | **Selectable experimental engine for preview and full/range jobs up to 720p** |
...
- The official RealBasicVSR EMA checkpoint strictly loads all 320 generator tensors and completed
  a real two-frame MPS inference smoke test.
